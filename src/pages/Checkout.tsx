...
import { ArrowLeft, Truck, Package, CreditCard, Wallet, Check } from "lucide-react";
import { z } from "zod";
...
const Checkout = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!parsed.success) {
      ...
      toast({ title: "Перевірте поля", description: first.message, variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (payment === "wayforpay") {
        toast({
          ...
          description: "Інтеграція WayForPay буде підключена наступним кроком.",
        });
      }

      toast({
        title: "Замовлення оформлено",
        description: `№ ${orderRef}. Доставка: ${deliveryName}. Менеджер зв'яжеться з вами.`,
      });
      clear();
      navigate("/");
    } finally {
  ...
  return (
    <div className="container py-10 max-w-6xl">
      <Link
        to="/cart"
        className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary transition-smooth mb-4"
      >
        ...
        <div className="space-y-8">
          ...
          <section className="p-6 rounded-2xl bg-card border border-border/60 shadow-soft">
            ...
            <p className="text-sm text-muted-foreground mt-3">
              Оплата доставки при отриманні за тарифами{" "}
              {delivery === "novaposhta" ? "Нової Пошти" : "MistExpress"}
            </p>
          </section>
          ...
          <section className="p-6 rounded-2xl bg-card border border-border/60 shadow-soft">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <DeliveryCard
                active={payment === "wayforpay"}
                onClick={() => setPayment("wayforpay")}
                icon={<CreditCard className="h-5 w-5" />}
                title="Оплата онлайн"
                subtitle="Картка через WayForPay"
              />
              <DeliveryCard
                active={payment === "cod"}
        ...
        <aside className="lg:sticky lg:top-24 h-fit p-6 rounded-2xl bg-card border border-border/60 shadow-card">
              ...
              <li key={product.id} className="flex gap-3 text-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-muted-foreground text-xs mt-0.5">
                    {quantity} × {formatUAH(product.price)}
                  </div>
                </div>
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span>{formatUAH(totalPrice)}</span>
            </div>
            <div className="flex justify-between">
...
const DeliveryCard = ({ active, onClick, icon, title, subtitle }: DeliveryCardProps) => (
  <button
    type="button"
    onClick={onClick}
    className={cn(
